    ...
    public partial class ChangePWDForm : Form
    {
        ...
        private void changeButton_Click(object sender, EventArgs e)
        {
            ...
            if (LoginForm.userType == "管理员")
            {
                ...
                accessOldPwd = LoginForm.access.searchUser("管理员", LoginForm.userName);
            }

            else //用户类型是 “一般用户”
            {
                userRegistry.isRegistryValueNameExist(LoginForm.userName, ref oldPwd);
                accessOldPwd = LoginForm.access.searchUser("一般用户", LoginForm.userName);
            }
                ...
                if (newPWDTextBox.Text.Trim() != String.Empty)
                {
                    ...
                    if (newPWDTextBox.Text.Trim() == newPWDTextBox2.Text.Trim())
                    {
                        if (LoginForm.userType == "管理员")  //“管理员修改密码”
                        {
                            ...
                            LoginForm.access.updatePassword("管理员", LoginForm.userName, md5.MD5Encryption(oldPWDTextbox.Text), md5.MD5Encryption(newPWDTextBox.Text.Trim()));
                        }
                        else  //“一般用户修改密码”
                        {
                            userRegistry.SetValue(LoginForm.userName, ras.Encryption(newPWDTextBox.Text.Trim()), RegistryValueKind.String);
                            LoginForm.access.updatePassword("一般用户", LoginForm.userName, md5.MD5Encryption(oldPWDTextbox.Text), md5.MD5Encryption(newPWDTextBox.Text.Trim()));
                        }
