...
namespace 串口调试助手
{
    public class Access
    {
        ...
        public Access(string filePath)
        {
            ...
            this.connection = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + filePath;  //新建数据库的路径  
        }
        ...
        public void creatDataDB()
        {
            ...
            if (!File.Exists(filePath))  //如果数据库不存在，那就添加数据库
            {
                try
                {
                    catalog.Create(connection);   //  ADOX.Catalog 对象，可以新建一个数据库 
        ...
        public List<string> searchTemperature(string tableName, string userType, string userName, string place, DateTime timeFrom, DateTime timeTo)
        {
            ...
            try
            {
                ...
                {
                    queryString = "select * from " + tableName + " where 用户类型 = '" + userType + "' and 用户名 = '" + userName + "'  ";
                }
